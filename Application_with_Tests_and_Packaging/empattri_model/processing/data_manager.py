...
from empattri_model import __version__ as _version
from empattri_model.config.core import DATASET_DIR, TRAINED_MODEL_DIR, config


##  Pre-Pipeline Preparation

def convert_yes_no_to_binary(dataframe: pd.DataFrame, column: str):
    df = dataframe.copy()
...
def pre_pipeline_preparation(*, data_frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if config.model_config.target in data_frame.columns:
        data_frame = convert_yes_no_to_binary(dataframe = data_frame, column = config.model_config.target)
